...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  for (int i = 0; i < height; i++) {
    ...
    DMA[3].cnt = width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_16 | DMA_SOURCE_FIXED;
  }
}
...
void drawFullScreenImageDMA(const u16 *image) {
  for (int i = 0; i < HEIGHT; i++) {
    ...
    DMA[3].cnt = WIDTH | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_SOURCE_INCREMENT | DMA_16;
  }
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int i = 0; i < height; i++) {
    DMA[3].src = image + OFFSET(i, 0, width);
    DMA[3].dst = videoBuffer + OFFSET(row + i, col, WIDTH);
    DMA[3].cnt = width | DMA_ON | DMA_DESTINATION_INCREMENT | DMA_16 | DMA_SOURCE_INCREMENT;
  }
}
